...
public class Main {
	public static void main(String[] args) throws Exception {
		if (args.length == 0) {
			System.out.println("Usage: java -jar travel-planner.jar <cities.csv> [--start CityName] [--return]");
			System.out.println("Or: mvn -q exec:java -Dexec.mainClass=com.hackathon.travel.Main -Dexec.args=\"data/cities.csv --start Victoria --return\"");
			return;
		}
	...
	private static List<City> loadCities(String csvPath) throws Exception {
		...
		for (String line : Files.readAllLines(Path.of(csvPath))) {
			line = line.trim();
